...
then
    ...
    NEW_KEY=$SCRIPT_DIR/contrib/mainapp_key_ecdsa-p256.pem
fi
...
echo CORRUPT: INSIDE SIGN VIA - $OLD_KEY
...
$IMGTOOL sign --key $OLD_KEY -H $HEADER_SIZE -S $SLOT_SIZE -M 2   --p1363 --custom-tlv 0xA3 $FW_DESCRIPTOR --security-counter $FW_SECURITY_CTR --align 32 --max-align 32 --pad-header -v $VERSION $INPUT_BINARY $OUTPUT_BINARY
$IMGTOOL dumpinfo $OUTPUT_BINARY

printf '\n\n%s\n' 'Wrapper (image #2) - signed with old key.'
result=$($IMGTOOL sign --key $OLD_KEY --custom-tlv 0xA2 $NEW_KEY -H $WRAPPER_HEADER_SIZE -S $SLOT_SIZE -M 2  --p1363 --custom-tlv 0xA3 $FW_DESCRIPTOR --security-counter $FW_SECURITY_CTR --align 32 --max-align 32 --pad-header -v $VERSION $OUTPUT_BINARY $OUTPUT_BINARY | grep Error)
if [ "$result" ]
then
    echo $result
else
